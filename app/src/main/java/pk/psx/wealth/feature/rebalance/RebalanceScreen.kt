...
fun RebalanceScreen(
    ...
    LaunchedEffect(state.portfolioId) {
        if (state.portfolioId != null && initializedFor != state.portfolioId) {
            reserve = state.reserve.stripTrailingZeros().toPlainString()
            minimumTrade = state.minimumTrade.stripTrailingZeros().toPlainString()
            allowSelling = state.allowSelling
